...
class Music {
    ...
    func fetchMusic(completion: @escaping ([[String:Any]]?, Error?) -> Void) {
        ...
        let dt = URLSession.shared.dataTask(with: URL.init(string: "https://orangevalleycaa.org/api/music")!) {(data, response, error) in completion(self.parseJSON(data: data!), error)}
        dt.resume()
    }
